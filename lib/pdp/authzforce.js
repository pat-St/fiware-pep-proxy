...
exports.checkPolicies = function (authToken, data, req) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (azf.custom_policy) {
      ...
      xml = getRESTPolicy(userName, roles, action, resource, appId);
    }
    return got
 ...
 * @return XML payload
 */
const getRESTPolicy = function (userName, roles, action, resource, appId) {
  ...
  if (roles.length > 0) {
    XACMLPolicy.Request.Attributes[0].Attribute[1] = {
      AttributeId: 'urn:oasis:names:tc:xacml:2.0:subject:role',
      IncludeInResult: 'false',
      AttributeValue: [
        // One per role
    ...
    for (const i in roles) {
      XACMLPolicy.Request.Attributes[0].Attribute[1].AttributeValue[i] = {
        //"AttributeId":"urn:oasis:names:tc:xacml:2.0:subject:role",
        //"IncludeInResult": "false",
        //"AttributeValue":{
        DataType: 'http://www.w3.org/2001/XMLSchema#string',
